 ...
 *
 * @pre Node type must be default constructible.
 */
...
/**
 * @brief A pool of reusable intrusive_node_t objects.
 *
 * @note This class is intended for single thread use case.
 *
 * @tparam Node                   The subject type for the pool.
 *                                Must be default constructible.
 * @tparam Intrusive_List_Member  Defines the hook for intrusive list
 *                                which is used to build free-list.
 * @tparam Page_Size              The size of a single page that contains
 *                                a bunch of nodes.
 */
...
public:
    ...
    intrusive_nodes_pool_t( intrusive_nodes_pool_t && )             = delete;
    intrusive_nodes_pool_t & operator=( intrusive_nodes_pool_t && ) = delete;
    ...
    /**
     * @brief Retire a given node.
     *
     * @pre The node must be the one that was initially allocated by this pool.
     */
